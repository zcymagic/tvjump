...
void ji1(double *data, double *par, double * H, int dims,int nobs, double *llf)
{
    
    int i,j,t,dims2,ind;
    double *mu_J,*ut,*Ht,*HI,*prob;
    double probs,lambdat;
    ...
    for(t=0;t<nobs;t++)
    {
        ...
        for(i=0;i<=NJUMP;i++)
        {                     
            ut[0] = data[t*dims]+(lambdat-i)*mu_J[0];
            Ht[0] = H[dims2*t]+HI[0]*i;  
            prob[i] = normp(ut,Ht)*possp(i,lambdat);
            probs  += prob[i];
...
{
	double *data, *par, *H,*llf,tmp;
	int  nobs,dims;
	
	/*  Check for proper number of arguments. */
	if(nrhs!=3)  /*Three inputs are data,para,H*/
